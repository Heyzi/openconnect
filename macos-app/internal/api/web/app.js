...
$('#recover').onclick = async () => { const button=$('#recover');button.disabled=true;button.textContent='Restoring…';try{await req('/api/v1/recover',{method:'POST'});await refresh()}catch(error){showStatusError(error.message,false)}finally{button.disabled=false;button.textContent='Restore network state'} };
...
async function poll() { try{await refresh()}catch(error){showStatusError('Status refresh failed: '+error.message,false)}finally{setTimeout(poll,2000)} }
